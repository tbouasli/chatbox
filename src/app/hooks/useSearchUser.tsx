...
export default function useSearchUser() {
  ...
  const indexDocRef = doc(
    firestore,
    "index",
    "user",
    "nickname",
    nickname.length === 0 ? "loading" : nickname
  );

  const [indexData, indexLoading] = useDocumentData(
    indexDocRef.withConverter(indexMapper)
  );
  const [userData, userLoading] = useDocumentData(
    indexData?.ref.withConverter(userMapper)
  );
